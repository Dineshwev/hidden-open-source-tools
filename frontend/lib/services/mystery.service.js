...
import { getFallbackApprovedFiles, pickFallbackMysteryFile } from "../utils/mysteryFallback.js";
...
export async function unlockMysteryFile(userId) {
  try {
    ...
    const fallbackFiles = getFallbackApprovedFiles();
    if (!fallbackFiles.length) {
      throw new AppError("No approved files are available yet", 404);
    }

    const selected = pickFallbackMysteryFile();
