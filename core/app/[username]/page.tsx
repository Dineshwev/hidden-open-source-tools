...
const reservedUsernames = new Set(['weekly-roundups']);

export default function CreatorProfile({ params }: { params?: { username?: string } }) {
  if (params?.username && reservedUsernames.has(params.username)) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-void via-[#0a0f23] to-[#1a0a2a] px-6 py-24 text-center text-white">
        <div className="mx-auto max-w-2xl rounded-3xl border border-white/10 bg-white/5 p-10">
          <p className="text-xs uppercase tracking-[0.3em] text-cyan-200/80">Not Found</p>
          <h1 className="mt-4 text-4xl font-bold">This page is not available</h1>
          <p className="mt-4 text-white/70">
            The requested path is reserved for another section of the site.
          </p>
        </div>
      </div>
    );
